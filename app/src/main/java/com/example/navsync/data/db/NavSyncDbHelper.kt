...
class NavSyncDbHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    suspend fun saveTripSession(record: TripSessionRecord) = withContext(Dispatchers.IO) {
        try {
            ...
            val id = db.insertWithOnConflict(TABLE_TRIPS, null, values, SQLiteDatabase.CONFLICT_REPLACE)
            Log.d(TAG, "Trip session saved id=$id sessionId=${record.sessionId} dist=${record.distanceKm}km")
        } catch (e: Exception) {
